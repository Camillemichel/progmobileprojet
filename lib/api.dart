...
class ComicVineApi {
  ...
  Future<dynamic> searchEpisodeByName(int id) async {
    ...
    final url = Uri.parse('https://comicvine.gamespot.com/api/episode/4070-${id.toString()}/?api_key=$_apiKey&format=json');
    final response = await http.get(url);
    if (response.statusCode == 200) {
      final jsonData = json.decode(response.body);
      return jsonData['results'];
  ...
  Future<dynamic> searchComicsByName(int id) async {
    final url = Uri.parse('https://comicvine.gamespot.com/api/issue/4000-$id/?api_key=$_apiKey&format=json');
    final response = await http.get(url);
    if (response.statusCode == 200) {
      final jsonData = json.decode(response.body);
      return jsonData['results'];
    } else {
      throw Exception('Erreur lors de la recherche du comics');
    }
  }
}

//https://comicvine.gamespot.com/api/issue/4000-6/?api_key=6db50ee6d46842bad12ce3ecbf244c7aae2f9041&format=json
